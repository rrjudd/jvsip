...
**********************************************************************/
/* $Id: vsip_tvectview_li.c,v 2.0 2003/02/22 15:19:09 judd Exp $ */
...
vsip_vview_li *vsip_tvectview_li(
      ...
      vsip_index j)
{
     vsip_vview_li *vview = (vsip_vview_li*)NULL;
     vsip_block_li *block = v->block;
     vsip_offset offset = v->offset;
     if(slice == VSIP_TVX){
        offset += i * v->z_stride + j * v->y_stride;
        vview = vsip_vbind_li(block,offset,v->x_stride,v->x_length);

     } else if(slice == VSIP_TVY) {
        offset += i * v->z_stride + j * v->x_stride;
        vview = vsip_vbind_li(block,offset,v->y_stride,v->y_length);

     } else if(slice == VSIP_TVZ){
        offset += i * v->y_stride + j * v->x_stride;
        vview = vsip_vbind_li(block,offset,v->z_stride,v->z_length);
