...
**********************************************************************/
/* $Id: vsip_vcreate_blackman_d.c,v 2.1 2003/04/22 02:19:58 judd Exp $ */
...
vsip_vview_d* (vsip_vcreate_blackman_d)(
  ...
  vsip_memory_hint h) {
   ...
   if(a == NULL) return (vsip_vview_d*)NULL;
   {
      ...
      while(n < N){
         *ap++ = 0.42 - 0.5 * (vsip_scalar_d)VSIP_COS_D(temp1 * (vsip_scalar_d) n) + 
                   0.08 * (vsip_scalar_d)VSIP_COS_D(temp2 * (vsip_scalar_d) n);
          n++;
      }
   }
   return a;
