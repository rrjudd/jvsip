...
static vsip_cscalar_f rcvdot_f( const vsip_vview_f* a, const vsip_cvview_f* b) {
    {
        vsip_length n = a->length;
        vsip_stride cbst = b->block->cstride;
        vsip_scalar_f 
        *apr = (vsip_scalar_f*) ((a->block->array) + a->block->rstride * a->offset),
                    ...
                    const vsip_cvview_f *yc)
{
    ...
    while(x->length < fir->M){
        vsip_cscalar_f a = rcvdot_f(h1,fir->s);
        vsip_cscalar_f b = rcvdot_f(h2,x);
    ...
    while(k < nout){ /* do the rest of the pieces */
        vsip_cvput_f(y,k++,rcvdot_f(fir->h,x));
        x->offset += oinc;
    }
    {
