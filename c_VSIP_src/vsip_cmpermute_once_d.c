...
void vsip_cmpermute_once_d(
   ...
   if(in == out) { /* in-place */
      ...
      vsip_scalar_d *ptr_im = in->block->I->array + in->offset * in->block->cstride;
      ...
      vsip_scalar_d *in_ptr_re = in->block->R->array + in->offset * in->block->cstride;
      vsip_scalar_d *out_ptr_re = out->block->R->array + out->offset * out->block->cstride;
      vsip_scalar_d *in_ptr_im = in->block->I->array + in->offset * in->block->cstride;
      vsip_scalar_d *out_ptr_im = out->block->I->array + out->offset * out->block->cstride;
      vsip_scalar_vi *in = p->block->array + p->offset;
      for(i=0; i<n_ind; i++){
         vsip_scalar_d *pin_re = in_ptr_re + in[i * p->stride] * in_ind_strd;
         vsip_scalar_d *pout_re = out_ptr_re + i * out_ind_strd;
         vsip_scalar_d *pin_im = in_ptr_im + in[i * p->stride] * in_ind_strd;
         vsip_scalar_d *pout_im = out_ptr_im + i * out_ind_strd;
         for(j=0; j < n_dta; j++){
            *pout_re = *pin_re;
            *pout_im = *pin_im;
