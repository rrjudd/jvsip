...
static void vsortip_d(const vsip_vview_d *in_out, 
                      vsip_sort_mode mode, 
                      vsip_sort_dir dir, 
                      vsip_bool fill,
...
/* begine permute */
static void mpermute_once_d(
                            const vsip_mview_d* in,
                            vsip_major major,
                            const vsip_vview_vi* p,
                            const vsip_mview_d* out){
    ...
    if(in == out) { /* in-place */
        vsip_scalar_d *ptr = in->block->array + in->offset * in->block->rstride;
        ...
        vsip_scalar_d *in_ptr = in->block->array + in->offset * in->block->rstride;
        vsip_scalar_d *out_ptr = out->block->array + out->offset * out->block->rstride;
        vsip_scalar_vi *in = p->block->array + p->offset;
        for(i=0; i<n_ind; i++){
            vsip_scalar_d *pin = in_ptr + in[i * p->stride] * in_ind_strd;
            vsip_scalar_d *pout = out_ptr + i * out_ind_strd;
            for(j=0; j < n_dta; j++){
                *pout = *pin;
                pout += out_dta_strd;
                pin += in_dta_strd;
...
vsip_scalar_d _sigma, _mu; vsip_index _i; \
...
vsip_index r,c; vsip_scalar_d t; vsip_index ind;\
for(r=_r; r<_cl; r++){ \
...
vsip_index _i; vsip_scalar_d *_ptr = _A + _r * _cs + _c * _rs; \
...
/* Bi-diagonalization */
static int bidiag_d(vsip_sv_d *obj){
    int retval = 0;
    vsip_scalar_d *v = obj->v,  *w=obj->w, *betaR=obj->betaR, *betaL=obj->betaL;
    vsip_length cl, rl, U_n, V_n; /* column, row length, size of U and V */
    vsip_stride   rs, cs, U_cs,U_rs,V_cs,V_rs;    /* strides */
    vsip_scalar_d *A = obj->A->block->array + obj->A->offset * obj->A->block->rstride;
    vsip_scalar_d *ptrA;
    vsip_scalar_d *U, *V;
    vsip_stride i,j;
    /* for A = U S V' */
    ...
    {  /* retrieve the diagonal and superdiagonal */
        /* store in d and f for use in VI_gk_d */
...
if(fabs(b) > fabs(a)) { \
_tau = -a/b; s = (vsip_scalar_d)(1.0/sqrt(1 + _tau * _tau)); c = s * _tau; } /* mod */\
else { \
_tau = -b/a; c = (vsip_scalar_d)(1.0/sqrt(1 + _tau * _tau)); s = c * _tau; } \
}}
...
register vsip_scalar_d _a11 = b11, _a12 = b12,\
_a21 = b21, _a22 = b22, _a23 = b23;\
register vsip_scalar_d _c,_s;      \
...
register vsip_scalar_d _a11 = b11, _a12 = b12, _a22 = b22;\
b11 = c * _a11 - s * _a12; \
b12 = c * _a12 + s * _a11; \
b21 = - s * _a22; \
b22 = c * _a22 ; \
...
register vsip_scalar_d _a11 = b11, _a21 = b21, _a12 = b12, _a22 = b22;\
register vsip_scalar_d _c,_s;      \
GIVENS_D(_a11,_a21,_c,_s);     \
b11 = _c * _a11 - _s * _a21; \
b12 = _c * _a12 - _s * _a22; \
 ...
 */
static vsip_length row_zero_d(vsip_scalar_d *d,
                              vsip_scalar_d *f,
                              vsip_scalar_d *c_l,
                              vsip_scalar_d *s_l,                
                              vsip_length n){ 
...
static vsip_scalar_d norm1(vsip_scalar_d *d,
                           vsip_scalar_d *f,
                           vsip_length n){
    vsip_index i;
    vsip_scalar_d retval,t=0;
    retval = fabs(d[0]);
    for(i=1; i<n; i++){
...
/* d is the diagonal; f is the supperdiagonal */
/*static vsip_scalar_d norm_fro(vsip_scalar_d *d,
 vsip_scalar_d *f,
 vsip_length n){
 vsip_index i;
...
static vsip_bool f_small( vsip_scalar_d d0,
                         vsip_scalar_d d1,
                         vsip_scalar_d f0,
                         vsip_scalar_d sn){
    vsip_bool ans;
...
vsip_scalar_d *_a1, *_a2, _t; vsip_stride _k;\
...
static void update(vsip_vview_d *buf, vsip_mview_d *U,vsip_scalar_d *c,vsip_scalar_d *s, vsip_index p,vsip_length n){
    vsip_index i;
    vsip_vview_d t = *buf;
    vsip_vview_d x1,x2;
    t.length = U->col_length;
    for(i=p; i<(p+n)-1; i++){
        VI_mcolview_d(U,i,&x1);
        VI_mcolview_d(U,(i+1),&x2);
        svmul_d(c[i-p], &x1 , &t);
        vsma_d(&x2, -s[i-p], &t, &t);
        svmul_d(s[i-p],&x1,&x1);
        vsma_d(&x2, c[i-p], &x1, &x2);
        vcopy_d_d(&t,&x1);
    }
    return;
}
...
static void zupdate(vsip_vview_d *buf, vsip_mview_d *U,vsip_scalar_d *c,vsip_scalar_d *s, vsip_index p,vsip_length n){
    vsip_index i;
    vsip_index q = p + (vsip_index) n;
    vsip_vview_d t = *buf;
    vsip_vview_d x1,x2;
    t.length = U->col_length;
    VI_mcolview_d(U, p, &x1);
    for(i=p; i<q; i++){
        VI_mcolview_d(U, (i+1), &x2);
        svmul_d(c[i-p], &x1 , &t);
        vsma_d(&x2, s[i-p], &t, &t);
...
static int gk_once_d(vsip_scalar_d *d,
                     vsip_scalar_d *f, 
                     vsip_scalar_d *c_l,
                     vsip_scalar_d *s_l,
                     vsip_scalar_d *c_r,
    ...
    int retval=0;
    vsip_index j;
    vsip_scalar_d t1,t2,dm,dn,f0,fm;
    vsip_scalar_d c_ret, s_ret; /* these need to be saved for update of U and V (for later) */
    /* G&V page 452 bottom */
    if(n > 2){
...
static int gk_d(vsip_sv_d *svd){
    int retval=0;
    vsip_length n = svd->L;
    vsip_index p=0,q=0;
    vsip_scalar_d *d = svd->d;
    vsip_scalar_d *f = svd->f;
    vsip_scalar_d *c_l=svd->c_l, 
    *s_l=svd->s_l, 
    *c_r=svd->c_r, 
    *s_r=svd->s_r;
    vsip_scalar_d chk0;
    ...
    for(;;){
        ...
        if((k == 0) && (f[0] == 0.0)){
            ...
            /* U -> U * P'; S -> P * S * P'; V' -> P * V' -> V * P' */
            {
                ...
                vi.length = vsip_vgetlength_d(svd->vd);
                vsortip_d(svd->vd,VSIP_SORT_BYVALUE,VSIP_SORT_DESCENDING,VSIP_TRUE,&vi);
                VRAMP((&vi_U));
                /*vsip_vramp_vi(0,1,&vi_U);*/
                vi_U.length = mV.row_length;
                vsip_vcopy_vi_vi(&vi,&vi_U);
                vi_U.length = mU.row_length;
                mpermute_once_d(&mU, VSIP_COL, &vi_U, &mU);
                mpermute_once_d(&mV, VSIP_COL, &vi, &mV);
            }
            break; /* done */
        }
        ...
        if(gkstep) { /* if no zeros were found do gkstep on submatrix */
            gk_once_d(&d[p], &f[p],c_l,s_l,c_r,s_r,(q - p + 1));
            ...
            while(p < q){
                ...
                gk_once_d(&d[p], &f[p],c_l,s_l,c_r,s_r,(p_f - p + 1));
                /* apply givens rotations to U and V */
                update(svd->buf,&mU,c_l,s_l,p,(q-p+1));
                update(svd->buf,&mV,c_r,s_r,p,(q-p+1));
                p = p_f;
...
int vsip_svd_d(
               ...
               vsip_vview_d *sv){
    int retval = 0;
    vsip_scalar_d *sv_ptr = sv->block->array;
