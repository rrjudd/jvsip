...
**********************************************************************/
/* $Id: VI_ccfftmip_d_loop.h,v 2.0 2003/02/22 15:18:29 judd Exp $ */

/* Use a loop of vsip_ccfftip_d.h to calculate fftm  */
                    ...
                    const vsip_cmview_d *y)
{
   ...
   if(fftm->major == VSIP_ROW){
      while(k < y->col_length){
         vsip_ccfftip_d(fft, VI_cmrowview_d(y,k,&zz));
         k++;
      }
   } else { /* must be column */
      while(k < y->row_length){
         vsip_ccfftip_d(fft, VI_cmcolview_d(y,k,&zz));
         k++;
      }
   }
}
