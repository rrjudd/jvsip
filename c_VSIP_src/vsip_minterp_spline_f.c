...
#define VINTERP_SPLINE( _t0, _y0, _spl, _xf0, _yf0) { \
   ...
   vsip_index i; \
   vsip_stride j; \
   vsip_scalar_f *t = _t0->block->array + _t0->offset * _t0->block->rstride; \
   vsip_scalar_f *_y = _y0->block->array + _y0->offset * _y0->block->rstride; \
   vsip_scalar_f *xf = _xf0->block->array + _xf0->offset * _xf0->block->rstride; \
   vsip_scalar_f *_yf = _yf0->block->array + _yf0->offset * _yf0->block->rstride; \
   vsip_scalar_f *z = _spl->z, *h=_spl->h, *b = _spl->b, *u = _spl->u, *v = _spl->v; \
   for(i=0; i<N-1; i++){ \
   ...
   for(i=0; i<M; i++){ \
      ...
      register vsip_scalar_f d,h,b,p,x; \
      x = xf[i * xf_st]; \
      j = N-2; \
