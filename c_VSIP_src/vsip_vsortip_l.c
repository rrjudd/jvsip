...
static
void
VI_qsort_l(vsip_vview_l *v, vsip_index left, vsip_index right, vsip_vview_vi *vi){
   vsip_index i,last;
   vsip_index iright;
   if(left >= right){
      return;
...
static
void
VI_qsortmag_l(vsip_vview_l *v, vsip_index left, vsip_index right, vsip_vview_vi *vi){
   vsip_index i,last;
   vsip_index iright;
   if(left >= right){
      return;
   ...
   last = left;
   for(i=left+1; i<= right; i++){
      if(labs(VGET(v,i)) < labs(VGET(v,left))){
         last++;
         SWAP(v,last,i);
         if(vi) SWAP_VI(vi,last,i);
      }
...
void vsip_vsortip_l(const vsip_vview_l *in_out, 
                  vsip_sort_mode mode, 
                  vsip_sort_dir dir, 
                  vsip_bool fill,
                  const vsip_vview_vi *vi0){
   vsip_vview_l in = *in_out;
