...
**********************************************************************/
/* $Id: vsip_tcreate_li.c,v 2.1 2004/09/21 02:07:25 judd Exp $ */
     ...
     vsip_memory_hint hint)
{
     vsip_block_li *block;
     vsip_tview_li *tview;
     block = VI_blockcreate_li(zlength * ylength * xlength,hint);
     if(block == NULL) return (vsip_tview_li*) NULL;
     tview = VI_tview_li();
     if(tview == NULL){
          VI_blockdestroy_li(block);
          return tview;
      }
     tview->block = block;
