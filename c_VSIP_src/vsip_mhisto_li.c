...
**********************************************************************/
/* $Id: vsip_mhisto_li.c,v 2.3 2004/07/18 15:28:14 judd Exp $ */

#include"vsip.h"
#include"vsip_mviewattributes_li.h"
#include"vsip_vviewattributes_li.h"
#include"VI_vfill_li.h"
...
void (vsip_mhisto_li)(
  ...
  vsip_scalar_li max,
  vsip_hist_opt opt,
  const vsip_vview_li* r) { /* histogram	*/
  {
    vsip_stride		str = 0;
    ...
    vsip_scalar_li *rp0 = rp, *rp_end = rp + p - 1 ;
    ...
    rp++;
    if(opt == VSIP_HIST_RESET) VI_vfill_l((vsip_scalar_li)0, r);
    while(n_mn-- > 0){
          vsip_length n = n_mj;
          while(n-- >0){
             if(*ap < min){
