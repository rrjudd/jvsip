...
**********************************************************************/
/* $Id: vsip_cblockadmit_d.c,v 2.1 2006/06/08 22:19:26 judd Exp $ */
...
int (vsip_cblockadmit_d)(
  ...
  vsip_scalar_bl update) {
   int blockadmit = (int)update; /* keep update from warning since don't use */
      ...
      #include"VI_cblockadmit_d_ds.h"
   #elif defined(VSIP_ALWAYS_SPLIT)
      #include"VI_cblockadmit_d_as.h"
   #elif defined(VSIP_ALWAYS_INTERLEAVED)
      #include"VI_cblockadmit_d_ai.h"
   #else
      #include"VI_cblockadmit_d_di.h"
   #endif
   return blockadmit;
}
