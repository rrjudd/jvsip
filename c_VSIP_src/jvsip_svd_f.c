...
#define mvprod_f(_A, _b, _r) {vsip_length nx=0, mx=0;\
vsip_scalar_f  *bp=(_b)->block->array+(_b)->offset * (_b)->block->rstride,\
...
#define svmul_f(alpha, _b, _r) {vsip_length n = (_r)->length;\
vsip_stride bst=(_b)->stride * (_b)->block->rstride, rst = (_r)->stride * (_r)->block->rstride;\
vsip_scalar_f *bp=(_b)->block->array+(_b)->offset * (_b)->block->rstride, *rp=(_r)->block->array+(_r)->offset * (_r)->block->rstride;\
...
static vsip_scalar_f svdMu_f(vsip_scalar_f d2,vsip_scalar_f f1,vsip_scalar_f d3,vsip_scalar_f* f2, vsip_scalar_f eps0)
{
    vsip_scalar_f mu;
...
static void svdStep_f(svdObj_f *svd)
{
    ...
    mu = svdMu_f(d2,f1,d3,&f2,svd->eps0);
...
{
    svd->eps0=(mnormFro_f((&svd->B))/(vsip_scalar_f)svd->B.row_length)*EPS;
    bidiag_f(svd);
    UmatExtract_f(svd);
    VHmatExtract_f(svd);
...
{
    svd->eps0=cmnormFro_f(&svd->B)/(vsip_scalar_f)(svd->B.row_length) * EPS;
    cbidiag_f(svd);
    cUmatExtract_f(svd);
    cVHmatExtract_f(svd);
...
int
vsip_svdprodu_f(const vsip_sv_f *svd, vsip_mat_op OpU, vsip_mat_side ApU, const vsip_mview_f *In)
{
