...
**********************************************************************/
/* $Id: VI_cblockadmit_f_ai.h,v 2.0 2003/02/22 15:18:28 judd Exp $ */
/* VI_cblockadmit_f always interleaved */
...
{ 
   if(b->Rp == (vsip_scalar_f*)NULL)
      ...
      if(b->Ip == (vsip_scalar_f*)NULL){ /* must be interleaved */
         ...
         b->R->array = b->data;
         b->I->array = b->data + 1;
         /* we care about the update flag here */
         if(update){ /* need to copy */
            vsip_length n = b->size;
            vsip_scalar_f *ptr_data = b->data,
                          *ptr_Rp = b->Rp,
                          *ptr_Ip = b->Ip;
            while(n-- > 0){
               *ptr_data = *ptr_Rp;
