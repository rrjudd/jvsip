...
**********************************************************************/
/* $Id: vsip_tmatrixview_l.c,v 2.0 2003/02/22 15:19:07 judd Exp $ */

#include"VI_support_priv_l.h"

vsip_mview_l *vsip_tmatrixview_l(
     const vsip_tview_l *v,
     vsip_tmslice slice,
     vsip_index index)
{
      vsip_mview_l *mview = (vsip_mview_l*)NULL;
      vsip_block_l *block = v->block;
      vsip_offset off = v->offset;
      if(slice == VSIP_TMYX){
          off += index * v->z_stride;
          mview = vsip_mbind_l(block,off,
