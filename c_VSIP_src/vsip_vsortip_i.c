...
static
void
VI_qsort_i(vsip_vview_i *v, vsip_index left, vsip_index right, vsip_vview_vi *vi){
   vsip_index i,last;
   vsip_index iright;
   if(left >= right){
      return;
   ...
   if(last > 0) VI_qsort_i(v,left,last-1,vi);
   VI_qsort_i(v,last+1,right,vi);
   return;
}
...
void vsip_vsortip_i(const vsip_vview_i *in_out, 
                  vsip_sort_mode mode, 
                  vsip_sort_dir dir, 
                  vsip_bool fill,
                  const vsip_vview_vi *vi0){
   vsip_vview_i in = *in_out;
   ...
   if(mode == VSIP_SORT_BYMAGNITUDE){
      VI_qsortmag_i(&in,(vsip_index) 0,(vsip_index)(in.length-1),vi);
   } else {
      VI_qsort_i(&in,(vsip_index) 0,(vsip_index)(in.length-1),vi);
   }
   return;
}
