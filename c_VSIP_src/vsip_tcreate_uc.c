...
**********************************************************************/
/* $Id: vsip_tcreate_uc.c,v 2.1 2004/09/21 02:07:25 judd Exp $ */
#define VI_TVIEW_UC_
     ...
     vsip_memory_hint hint)
{
     vsip_block_uc *block;
     vsip_tview_uc *tview;
     block = VI_blockcreate_uc(zlength * ylength * xlength,hint);
     if(block == NULL) return (vsip_tview_uc*) NULL;
     tview = VI_tview_uc();
     if(tview == NULL){
          VI_blockdestroy_uc(block);
          return tview;
      }
     tview->block = block;
