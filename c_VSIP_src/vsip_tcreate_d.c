...
**********************************************************************/
/* $Id: vsip_tcreate_d.c,v 2.1 2004/09/21 02:07:24 judd Exp $ */
     ...
     vsip_memory_hint hint)
{
     vsip_block_d *block;
     vsip_tview_d *tview;
     block = VI_blockcreate_d(zlength * ylength * xlength,hint);
     if(block == NULL) return (vsip_tview_d*) NULL;
     tview = VI_tview_d();
     if(tview == NULL){
          VI_blockdestroy_d(block);
          return tview;
      }
     tview->block = block;
