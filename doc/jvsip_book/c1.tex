...
The \jv distribution contains a C signal processing library implementing (most of) the \ttbf{C VSIPL} specification. \jv also contains a python vsip module encapsulating the C library.  Once the {\ttbf{vsip}} module was done a new module called {\ttbf{vsiputils}} was done to provide function overloading and to reduce the name space.  One of the main purposes of the {\ttbf{vsiputils}} module was to help me to learn python programing; but a lot of work was done there and the module still survives although it may go away in the future.  Eventually I got around to defining python classes and created the \pyjv module.  In this document we mainly treat the python interface defined in the \pyjv module but you should be aware other modules exist.
...
\begin{minipage}[t][10cm][t]{.45\textwidth}
