...
\\\cvsiplh 
\begin{cfuncs}
\multicolumn{1}{c}{\Ts\rmfamily \bfseries Create CHOL Object\Bs}\\ \hline
...
\multicolumn{1}{c}{\Ts\rmfamily \bfseries Destroy CHOL Object\Bs}\\ \hline
...
\multicolumn{1}{c}{\Ts\rmfamily \bfseries Calculate CHOL Decomposition\Bs}\\ \hline
...
\multicolumn{1}{c}{\Ts\rmfamily \bfseries Solve Using Calculated CHOL Decomposition\Bs}\\ \hline
...
\multicolumn{1}{c}{\Ts\rmfamily \bfseries Fill Attribute Structure With Current Attributes\Bs}\\ \hline
...
For class methods table we assume we have created an CHOL object we call \ttbf{cholObj} and we have an input matrix \ttbf{view A} compliant with \ttbf{cholObj} and some compliant \ttbf{view B} where we want to solve for the unknown \ttbf{X} in $A X = B $.}\\[6pt]}
%
\begin{center}\begin{tabular}{|l|l|}
\multicolumn{2}{c}{\parbox[t]{.85\textwidth}{\center{\rmfamily \bfseries CHOL Decomposition Methods\vspace{.2cm}}}\Bs} \\ \hline
\ttbf{cholObj.chold(A)} & \parbox[t]{.58\textwidth}{Decompose matrix \ttbf{A}. The decomposition is stored in the \ttbf{cholObj} but \ttbf{view A} may be overwritten so use a copy if you want to preserve \ttbf{A}\Bs}\\\hline
\ttbf{cholObj.solve(op,XB)} & \parbox[t]{.58\textwidth}{Solve problem $\text{op}(A) X = B$ in-place where \ttbf{XB} is input/output \ttbf{view} and \ttbf{op} is matrix operator flag.\Bs}\\\hline
\ttbf{cholObj.size} & \parbox[t]{.58\textwidth}{ Property. Returns integer length size of square matrix \ttbf{CHOL} object will decompose.\Bs}\\\hline
\ttbf{cholObj.singular} & \parbox[t]{.58\textwidth}{Property. Returns \ttbf{True} if singular; \ttbf{False} if inverse exists.\Bs}\\\hline
\ttbf{cholObj.type} & \parbox[t]{.58\textwidth}{Returns string indicating CHOL type.\Bs}\\\hline
\ttbf{cholObj.vsip} & \parbox[t]{.58\textwidth}{Returns C VSIPL CHOL instance.\vspace*{.1cm}\Bs}\\\hline
...
%%%
\multicolumn{2}{c}{\Ts\parbox[t]{.6\textwidth}{\center{\rmfamily \bfseries Significant Part of Matrix (\ttbf{sp})}}}\Bs\\\hline
