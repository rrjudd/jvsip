...
\item{Other parameters are set to there default value or calculated.\vspace{.2cm}}
\end{itemize}}\\
\clearpage
...
\hline\end{tabular}}
\clearpage\hspace*{.8cm}{\textbf{FIR Class Methods}\\
\hspace*{1.1cm} \parbox[t]{.88\textwidth}{Below we assume we have created an FIR object we call \ttbf{firObj} and we have an input \ttbf{view x} compliant with \ttbf{firObj} and a compliant output \ttbf{view y}.\vspace{.2cm}}
\newline\hspace*{1.2cm}
\parbox[t]{.85\textwidth}{To filter the data \ttbf{x} and place the results in \ttbf{y} we do \\*\hspace*{.5cm}\ttbf{fftObj.flt(x,y)}\\
To retrieve the length of input vector \ttbf{view} the filter expects in \ttbf{x} we do\vspace{.1cm}\\*\hspace*{.5cm} \ttbf{n=firObj.length}\\
To retrieve the decimation factor the filter was created with we do\\*\hspace*{.5cm} \ttbf{n=firObj.decimation}\\
It is possible if we are saving state and using the filter on consecutive input \ttbf{view}s to filter data consecutively that the length of the output will vary. To retrieve the number of output values placed in \ttbf{y} we do\vspace{.1cm}\\*\hspace*{.5cm} \ttbf{n=firObj.lengthOut}\\
To recover the filter type (a string) we do \\*\hspace*{.5cm} \ttbf{t=firObj.type}\\
To recover the C VSIP filter object associated with the pyJvsip filter we do\\*\hspace*{.5cm}\ttbf{vsipObj=firObj.vsip}\\
}