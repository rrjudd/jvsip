...
def am(a,b,c,d):
    ...
    assert 'pyJvsip' in repr(a),'Argument one must be a pyJvsip view object in am'
    t1=a.type
    ...
    assert 'pyJvsip' in repr(c),'Argument three must be a pyJvsip view object in am'
    assert __isSizeCompatible(a,c),'Size error in add-multiply (am)'
    t3=c.type
    assert a.type==c.type,'View types must agree in am'
    assert 'pyJvsip' in repr(d),'Argument four must be a pyJvsip view object in am'
    assert __isSizeCompatible(a,d),'Size error in add-multiply (am)'
    ...
    if 'scalar' in t2 and 'cvview' in t1:
        t2='cscalar'
    t=t1+t2+t3+t4
    assert t in f, 'Type <:'+t+':> not recognized for am'
    if 'cscalar' in t:
        if 'cvview_d' in t:
...
def ma(a,b,c,d):
    ...
    if isinstance(c,int) or isinstance(c,float):
        ...
        c0=c.vsip
    t=t1+t2+t3+t4
    assert t in f, 'Type <:'+t+':> not recognized for ma'
...
def msb(a,b,c,d):
    ...
    assert 'pyJvsip' in repr(a),'Argument one must be a pyJvsip view object in msb'
    assert 'pyJvsip' in repr(b),'Argument two must be a pyJvsip view object in msb'
    assert 'pyJvsip' in repr(c),'Argument three must be a pyJvsip view object in msb'
    assert 'pyJvsip' in repr(d),'Argument four must be a pyJvsip view object in msb'
    ...
    assert t in f, 'Type <:'+t+':> not recognized for msb'
    f[t](a.vsip,b.vsip,c.vsip,d.vsip)
    return d
