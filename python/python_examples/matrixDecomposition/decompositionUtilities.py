...
def bidiag(A): # m >= n
    ...
    for i in range(n-1):
        ...
        if i < n-2:
            ...
            v=houseVector(x).conj #this seems to work and is less computations
            v /= v[0]
...
def bidiagExtract(B):
    """
      B=bidiagExtract(B0)
        returns, out of place, a matrix with the bidiagonal entries. Input matrix
        is one produced by B0=bidiag(A)
    """
    B0=B.empty.fill(0.0)
...
def UmatExtract(B):
    """
      U=UmatExtract(B0)
        ...
        decomposition A=UBV^H given the result of bidiag routine B0=bidiag(A)
    """
    m=B.collength
    n=B.rowlength
    U=eye(B.type,m)
