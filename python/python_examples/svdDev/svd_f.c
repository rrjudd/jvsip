...
/* note (here) diag_sv_f only works for svd algorithm*/
...
/* x and v are already created. v compact with zero offset. Block length of v must be >= length of x */
static vsip_vview_f* vclone_f(vsip_vview_f*x, vsip_vview_f *v){
    vsip_vputlength_f(v,vsip_vgetlength_f(x));
    vsip_vcopy_f_f(x,v);
    return v;
} /* OK */
/* convenience functions for creating identity matrices */
...
static void svdBidiag_f(svdObj_f *svd)
{
    ...
    vsip_mview_f *B = svd->B;
    vsip_scalar_f sv = mnormFro_f(B)/(vsip_scalar_f)vsip_mgetrowlength_f(B);
    bidiag_f(svd); 
